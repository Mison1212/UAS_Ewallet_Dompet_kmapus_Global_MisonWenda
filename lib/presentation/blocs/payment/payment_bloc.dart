...
class PaymentBloc extends Bloc<PaymentEvent, PaymentState> {
  ...
  Future<void> _onTopup(PaymentTopupRequested event, Emitter<PaymentState> emit) async {
    ...
    try {
      ...
      emit(PaymentError('Sesi Anda telah berakhir. Silakan login ulang.'));
    } on ServerFailure catch (e) {
      emit(PaymentError(e.message));
    } on NetworkFailure catch (e) {
      emit(PaymentError(e.message));
    } catch (e) {
      emit(PaymentError('Terjadi kesalahan tak terduga.'));
    }
  }
  ...
  Future<void> _onTransfer(PaymentTransferRequested event, Emitter<PaymentState> emit) async {
    ...
    try {
      ...
      // Sesi expired — tampilkan sebagai error biasa agar user tahu harus login ulang
      emit(PaymentError('Sesi Anda telah berakhir. Silakan login ulang.'));
    } on ServerFailure catch (e) {
      emit(PaymentError(e.message));
    } on NetworkFailure catch (e) {
