...
class _PinPageState extends State<PinPage> {
  String _pin = '';
  String _enteredPin = ''; // simpan PIN yang dimasukkan user
  bool _busy = false;
  bool _hasError = false;

  void _onComplete(String pin) {
    // Simpan PIN yang dimasukkan, lalu proses pembayaran
    _enteredPin = pin;
    setState(() => _busy = true);
    _processPayment();
  }
  ...
  void _processPayment() {
    ...
    // Gunakan PIN yang dimasukkan user sebagai kode OTP
    final pinCode = _enteredPin;
    // Hapus dari memori setelah diambil (keamanan)
    _enteredPin = '';
    ...
    if (kind == 'transfer') {
      context.read<PaymentBloc>().add(PaymentTransferRequested(
        ...
        otpCode: pinCode, // PIN asli dari user, bukan hardcoded
      ...
      // Pembayaran QRIS → gunakan transfer endpoint dengan PIN user
      context.read<PaymentBloc>().add(PaymentTransferRequested(
        amount: (flow['amount'] as num).toDouble(),
      ...
      listener: (context, state) {
        if (state is PaymentTransferSuccess) {
          ...
          context.go('/success', extra: {
            ...
            if (widget.flowData['callback_url'] != null)
              'callback_url': widget.flowData['callback_url'],
          });
          ...
          // PIN/OTP salah — reset form agar user bisa coba lagi
          setState(() { _busy = false; _hasError = true; _pin = ''; _enteredPin = ''; });
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Saldo tidak cukup. Saldo: ${CurrencyFormatter.format(state.balance)}, Tagihan: ${CurrencyFormatter.format(state.amount)}'),
              backgroundColor: AppColors.red,
            ),
          );
          Future.delayed(const Duration(milliseconds: 800), () {
          ...
          setState(() { _busy = false; _pin = ''; _enteredPin = ''; });
          ScaffoldMessenger.of(context).showSnackBar(
            ...
            children: [
              Align(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    final callbackUrl = widget.flowData['callback_url'] as String?;
                    if (callbackUrl != null) {
