...
class MerchantCheckoutPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Determine dynamic amount
    final parsedAmount = double.tryParse(amount ?? '0') ?? 0.0;
    
    // If we have an amount from deep link, we use it directly. Otherwise use dummy data.
    final hasDynamicData = parsedAmount > 0;
    
    final items = hasDynamicData 
        ? [{'name': 'Pembayaran Order', 'qty': 1, 'price': parsedAmount}]
        : [
            {'name': 'Kemeja Flanel Oversize', 'qty': 1, 'price': 159000.0},
            {'name': 'Tumbler Stainless 750ml', 'qty': 2, 'price': 45000.0},
          ];
          
    final ship = hasDynamicData ? 0.0 : 12000.0;
    final subtotal = items.fold(0.0, (s, i) => s + (i['price'] as double) * (i['qty'] as int));
    final total = subtotal + ship;
    final displayOrderId = orderId ?? 'TB-2026-88142';
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Text('Pesanan #$displayOrderId',
          ...
          Container(
            ...
            child: AppButton(
              ...
              onPressed: () => context.go('/pin', extra: {
                'kind': 'deeplink',
                'description': 'Pesanan #$displayOrderId',
                'amount': total,
