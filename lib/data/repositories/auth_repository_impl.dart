...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<({UserEntity user, String token})> register(String nama, String email, String password) async {
    try {
      final result = await _remote.register(nama, email, password);
      // Jangan simpan token/user karena harus verifikasi email link dulu sebelum login
  ...
  Future<({UserEntity user, String token})> login(String email, String password) async {
    try {
      ...
      await _local.saveAuthVerified(true); // Auto verified in this backend
      return (user: result.user, token: result.token);
    } on UnauthorizedException catch (e) {
      throw AuthFailure(e.message, errorCode: e.errorCode);
    } on ServerException catch (e) {
  ...
  Future<void> verifyEmailOtp(String code) async {
    try {
      ...
      await _local.saveAuthVerified(true);
    } on ServerException catch (e) {
      throw ServerFailure(e.message, errorCode: e.errorCode);
    } on NetworkException catch (e) {
      throw NetworkFailure(e.message);
